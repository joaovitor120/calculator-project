...
def ensure_integer_result_simple(result, num1,num2, op_symbol,index_min, index_med, op_min, op_max, difficulty=None, complex=False):
    if not isinstance(result, int): #if result not int
        ...
        if op_symbol == "/":
            ...
            if validates_num2 and complex:
                ...
                while num2 not in validates_num2:
                    num3, num4, op_symbol_exp_2, num2 = generate_challenge(difficulty, op_min, op_max, last_num=True)
    ...
    result = operations_type[op_symbol](num1, num2)
    return num3, num4, op_symbol_exp_2, num2, op_symbol, result if complex else num2, op_symbol, result
...
def challenge_complex(difficulty, op_min, op_max):
    ...
    num3, num4, op_symbol_exp_2, result_exp_2, op_symbol_center, final_result = ensure_integer_result_simple(final_result,result_exp_1,result_exp_2,op_symbol_center,ranges_min(difficulty), ranges_med(difficulty),op_min, op_max,difficulty, complex=True)
    #print(op_symbol_2)
    ...
    answer_retry_loop(result_exp_1,result_exp_2,op_symbol_center,user_result_answer,final_result,expression) 
    end_time = end_timer()
    execution_time = end_time - start_time
